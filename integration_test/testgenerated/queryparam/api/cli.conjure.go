...
func (c TestServiceCLICommand) testService_Echo_CmdRun(cmd *cobra.Command, _ []string) error {
	...
	switch {
	case listParamRaw == "@-":
		if err := codecs.JSON.Decode(cmd.InOrStdin(), &listParamArg); err != nil {
	...
	case strings.HasPrefix(listParamRaw, "@"):
		...
		if err := listParamArgReader.Close(); err != nil {
			return werror.WrapWithContextParams(ctx, err, "failed to close file for argument listParam")
		}
	default:
		if err := codecs.JSON.Unmarshal([]byte(listParamRaw), &listParamArg); err != nil {
			return werror.WrapWithContextParams(ctx, err, "invalid value for listParam argument")
		}
	}
