...
func (c TestServiceCLICommand) testService_BinaryAlias_CmdRun(cmd *cobra.Command, _ []string) error {
	...
	var bodyArg httpclient.RequestBody
	switch {
	case bodyRaw == "@-":
		bodyArg = httpclient.RequestBodyStreamOnce(func() io.ReadCloser {
			return io.NopCloser(cmd.InOrStdin())
		})
	case strings.HasPrefix(bodyRaw, "@"):
		bodyArg = httpclient.RequestBodyStreamWithReplay(func() (io.ReadCloser, int64, error) {
			bodyArgReader, err := os.Open(strings.TrimSpace(bodyRaw[1:]))
			if err != nil {
				return nil, 0, werror.WrapWithContextParams(ctx, err, "failed to open file for argument body")
			}
			fileInfo, err := bodyArgReader.Stat()
			if err != nil {
				return nil, 0, werror.WrapWithContextParams(ctx, err, "failed to stat file for argument body")
			}
			return bodyArgReader, fileInfo.Size(), nil
		})
	default:
		bodyArgBytes, err := base64.StdEncoding.DecodeString(bodyRaw)
		if err != nil {
			return werror.WrapWithContextParams(ctx, err, "invalid value for body argument")
		}
		bodyArg = httpclient.RequestBodyInMemory(bytes.NewReader(bodyArgBytes))
	}
