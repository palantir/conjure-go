...
func (_m *TestServiceClient) PutBinary(ctx context.Context, myParamArg httpclient.RequestBody) (io.ReadCloser, error) {
	ret := _m.Called(ctx, myParamArg)
	var r0 io.ReadCloser
	if rf, ok := ret.Get(0).(func(context.Context, httpclient.RequestBody) io.ReadCloser); ok {
		r0 = rf(ctx, myParamArg)
	} else if v := ret.Get(0); v != nil {
		r0 = v.(io.ReadCloser)
	}
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, httpclient.RequestBody) error); ok {
		r1 = rf(ctx, myParamArg)
	} else {
		r1 = ret.Error(1)
	}
