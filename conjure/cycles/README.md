...
- A **conjure package** is a package according to the unmodified conjure definition.
- A **Go package** is a package according to the modified conjure definition (that is, it's ready to be compiled to Go).
...
- If the type represented by node `u` is an alias to a primitive data structure that references another type represented
