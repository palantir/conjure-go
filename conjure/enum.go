...
func astForEnum(ctx types.TypeContext, enumDefinition spec.EnumDefinition) []astgen.ASTDecl {
	...
	unmarshalDecl := enumUnmarshalTextAST(ctx, enumDefinition)

	return []astgen.ASTDecl{typeDef, valsDecl, unmarshalDecl}
}

func enumUnmarshalTextAST(ctx types.TypeContext, e spec.EnumDefinition) astgen.ASTDecl {
	toCamelCase := varcaser.Caser{From: varcaser.ScreamingSnakeCase, To: varcaser.UpperCamelCase}.String

	ctx.AddImports("strings")
