		...
		doAppendBodyRequestParam := func(block *jen.Group) {
			if body.Type.IsBinary() {
				...
				if body.Type.IsNamed() {
					...
					appendRequestParams(block, snip.CGRClientWithJSONRequest().Call(snip.CJNewMarshalerTo().Types(body.Type.Code(), jsonv2.GetCJMarshalerType(body.Type)).Call(jen.Id(bodyArg))))
				}
			} else {
				appendRequestParams(block, snip.CGRClientWithJSONRequest().Call(jen.Id(bodyArg)))
			}
	...
	if endpointDef.Returns != nil {
		...
		if returnType.IsBinary() {
			...
			if returnType.IsNamed() {
				...
				var unmarshalerType *jen.Statement
				if returnType.IsOptional() || returnType.IsCollection() {
					unmarshalerType = snip.CJNewUnmarshalerFrom().Types(returnType.Code(), jsonv2.GetCJUnmarshalerType(returnType))
				} else {
					// Wrap the response unmarshaler in an optional handler so we can distinguish between
					// missing and present-but-zero-value responses.
					unmarshalerType = snip.CJNewUnmarshalerFrom().Types(jen.Op("*").Add(returnType.Code()), jsonv2.GetCJUnmarshalerType(&types.Optional{Item: returnType}))
				}
				appendRequestParams(methodBody, snip.CGRClientWithJSONResponse().Call(unmarshalerType.Call(jen.Op("&").Id(returnValVar))))
			}
		} else {
			appendRequestParams(methodBody, snip.CGRClientWithJSONResponse().Call(jen.Op("&").Id(returnValVar)))
		}
