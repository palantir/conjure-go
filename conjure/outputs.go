...
var _ OutputFileWriter = &OutputFile{}
...
func (f *OutputFile) Write() error {
	...
	return writeFileHelper(f.absPath, body)
}
...
func writeFileHelper(absPath string, content []byte) error {
	if err := os.MkdirAll(filepath.Dir(absPath), 0755); err != nil {
		return errors.Wrapf(err, "failed to create parent directory for Go file output %s", absPath)
	}
	if err := ioutil.WriteFile(absPath, content, 0644); err != nil {
		return errors.Wrapf(err, "failed to write Go file output to %s", absPath)
	}
	return nil
}
...
type stringOutputFile struct {
	absPath string
	content string
}

var _ OutputFileWriter = &stringOutputFile{}

func (f *stringOutputFile) AbsPath() string {
	return f.absPath
}

func (f *stringOutputFile) Write() error {
	return writeFileHelper(f.absPath, addHeaderComment([]byte(f.content)))
}
