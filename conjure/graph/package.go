...
func RemovePackageCycles(def spec.ConjureDefinition) (spec.ConjureDefinition, error) {
	...
	for _, componentGroups := range mergedComponents {
		...
		// Sort type groups by descending order of size so we make less modifications on bigger groups
		sort.SliceStable(typeGroups, func(i, j int) bool {
			if len(typeGroups[i]) != len(typeGroups[j]) {
				return len(typeGroups[i]) > len(typeGroups[j])
			}
