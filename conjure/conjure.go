...
func GenerateOutputFiles(conjureDefinition spec.ConjureDefinition, cfg OutputConfiguration) ([]*OutputFile, error) {
	...
	for _, pkg := range def.Packages {
		...
		if len(def.Extensions) > 0 {
			embedGoFile := filepath.Join(pkg.OutputDir, "embed.conjure.go")
			embedGoContent := []byte(fmt.Sprintf(`// This file was generated by Conjure and should not be manually edited.
...
`, pkg.PackageName))
			...
			extensionsFile := filepath.Join(pkg.OutputDir, "extensions.conjure.json")
			...
			files = append(files,
				newRawFile(extensionsFile, extensionsContent),
				newRawFile(embedGoFile, embedGoContent),
			)
		}
